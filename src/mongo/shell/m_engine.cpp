...
#include "mongo/shell/m_engine.h"
...
#include "mongo/util/my_icu_init.h"
...
void initMyEngine() {
    mongo::StaticInit();
    mongo::initMyVersion();
    mongo::initMyHttpClient();
    mongo::initDiagnosticInfo();
    mongo::initMyIndexKeyVaildate();
    mongo::initMyLogicalTimeV();
    mongo::initMyFtsIndexFormat();
    mongo::initMyExpression();
    mongo::initMyShellOptions();
    mongo::initMyOptionsParser();
    mongo::initMymongodbcr();
    mongo::initMyOptionsParserInit();
    mongo::initMyErrorExtraInfo();
    mongo::initMyExpressionTrigonometric();
    mongo::initMyShutDownProgress();
    mongo::initMytxnRetryOldInfo();
    mongo::initMyCannotConvertInfo();
    mongo::initMyCollectionUuidMisInfo();
    mongo::doc_validation_error::initMyDocVailDationError();
    mongo::initMyChangeStreamInvalidationInfo();
    mongo::initMyChangeStreamStartAfterInfo();
    mongo::initMyChangeStreamToInfo();
    mongo::initMyTenantMigrationConFlictInfo();
    mongo::initMyDuplicateKeyErrorInfo();
    mongo::initMyResolvedView();
    mongo::initMyCannotImplicitlyCreateCollectionInfo();
    mongo::initMyShardCannotRefreshDueToLocksHeldInfo();
    mongo::initMyShardInvalidatedForTargetingInfo();
    mongo::initMyStaleException();
    mongo::initMyWouldChangeOwningShardInfo();
    mongo::initMyBatchedCommandResponse();

    mongo::initMyIcuInit();
    // mongo::initMyIcuInitStub();
    // mongo::initSslManagerNone();
    mongo::initMyLDAPCumulativeOperationStats();
    mongo::initMyServiceExecutorSynchronous();
    mongo::initMyServiceExecutorReserved();
    mongo::transport::initMyServiceExecutorFixed();
    mongo::initMySnappyMessageCompressor();
    mongo::initMyZlibMessageCompressor();
    mongo::initMyZstdMessageCompressor();
    mongo::initMyExpressionJsEmit();
    mongo::initMyExpressionTestApiVersion();
    mongo::initMyExpressionFunction();
    mongo::initMyProcessinfo();
    mongo::initMyNativeSaslClientSession();
    mongo::initMongoMain();
}

namespace {

const int myInitEngine = (mongo::initMyEngine(), 0);

bool gotInterrupted = false;
bool inMultiLine = false;
bool lastLineSuccessful;
...
std::map<std::string, std::unique_ptr<mongo::Scope>> scopes;
...
extern "C" void quitNicely(int sig) {
   // shutdown(EXIT_CLEAN);
   std::cout << "pos-x signal: " << sig << std::endl;
}

extern "C" void printSignal(int sig) {
   std::cout << "pos-x signal: " << sig << std::endl;
}
...
void MEngine::initGlobal() {
    setupSignalHandlers();
    setupSignals();

    auto resVector = std::vector<std::string>(0);
    mongo::runGlobalInitializersOrDie(resVector);
...
int MEngine::init(char* host, int port, char* username, char* password, char* connId) {
    try {
        initMtx.lock();
        mongo::shell_utils::dbConnect = "";
        std::stringstream shellGlobalParamsUrl;

        if (strcmp(username, "") == 0 && strcmp(password, "") == 0) {
            // 没有输入用户和密码的情况
            shellGlobalParamsUrl << "mongodb://" << host << ":" << port;
        } else {
            shellGlobalParamsUrl << "mongodb://" << username << ":" <<  password << "@" << host << ":" << port;;
        }
        shellGlobalParamsUrl << "/?authSource=admin";
        shellGlobalParams.url = shellGlobalParamsUrl.str();
        ...
        shellGlobalParams.username = username;
        shellGlobalParams.password = password;
        shellGlobalParams.authenticationDatabase = "admin";
        ...
        // 因为不需要在命令行展开所以quiet按true走，即serverGlobalParams.quiet = true
        if (!shellGlobalParams.nodb) {  // connect to db
            ...
            std::cout << "usr:" << parsedURI.getUser() << std::endl;
            std::cout << "psw:" << parsedURI.getPassword() << std::endl;
        ...
        std::unique_ptr<mongo::Scope> myScope(mongo::getGlobalScriptEngine()->newScope());

        // std::unique_ptr<mongo::Scope> scope = std::move(myScope);
        scopes[connId] = std::move(myScope);
        shellMainScope = scopes[connId].get();
        ...
        if (!shellGlobalParams.norc) {
            ...
            if (!rcLocation.empty() && ::mongo::shell_utils::fileExists(rcLocation)) {
                hasMongoRC = true;
                if (!scopes[connId]->execFile(rcLocation, false, true)) {
...
const char* MEngine::exec(char* command, char* connId) {
    // std::unique_ptr<mongo::Scope> scope = scopes[connId];
    std::string exeRes = "";
    if (this->errorCodes.count(connId) != 1) { 
        exeRes = "-10";
        const char *result;
        result = strdup(exeRes.c_str());
        return result;
    }
    
    try {
        this->errorCodes[connId] = 0;
        inMultiLine = false;
        gotInterrupted = false;
        std::string code = command;
        promptType = scopes[connId]->type("prompt");
        if (promptType == String) {
            prompt = scopes[connId]->getString("prompt");
        } else if ((promptType == Code) && execPrompt(*scopes[connId], "prompt", prompt)) {
        } else if (execPrompt(*scopes[connId], "defaultPrompt", prompt)) {
        } else {
            prompt = "> ";
        }
        bool wascmd = false;
        {
            ...
            if (cmd.find("\"") == std::string::npos) {
                try {
                    lastLineSuccessful =
                        scopes[connId]->exec(std::string("__iscmd__ = shellHelper[\"") + cmd + "\"];",
                                    "(shellhelp1)",
                                    false,
                                    true,
                                    true);
                    if (scopes[connId]->getBoolean("__iscmd__")) {
                        lastLineSuccessful =
                            scopes[connId]->execAndGetResult(std::string("shellHelper( \"") + cmd + "\" , \"" +
                    ...
                    exeRes = e.what();
                    this->errorCodes[connId] = -1;
                }
            }
        }
        if (!wascmd) {
            try {
                lastLineSuccessful = scopes[connId]->exec(code, "(shell)", false, true, false);
                if (lastLineSuccessful) {
                    scopes[connId]->execAndGetResult(
...
void MEngine::destroy(char* connId) {
    // killOps();
    this->errorCodes.erase(connId);
    // scopes[connId]
    scopes.erase(connId);
    // delete scope;
}
}  // namespace mongo
//  rm  libexecute.a && ar -crsv libexecute.a shell/*.o